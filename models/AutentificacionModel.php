...
class AutentificacionModel{
    ...
    public function doLogin($usuario, $password){
        ...
        $consulta="SELECT usuario_personal, id_personal FROM personal WHERE usuario_personal='$usuario' AND password_personal= md5('$password') AND estado_usuario_cliente= 1";
    ...
    public function doLoginCliente($usuario, $password){
        ...
        $consulta="SELECT id_cliente, usuario_cliente, estado_usuario_cliente FROM cliente WHERE usuario_cliente='$usuario' AND password_cliente= md5('$password') AND estado_usuario_cliente= 1";
        $conexion= Database::connect();
        $resultado=$conexion->query($consulta);
        $loginvalido='0';
